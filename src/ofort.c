...
typedef struct {
    char name[256];
    char var_names[OFORT_MAX_PARAMS][256];
    int n_vars;
} OfortNamelist;
...
static const char *procedure_ref_name(const OfortValue *v) {
    ...
    if (!v || v->type != FVAL_CHARACTER || !v->v.s) return NULL;
    if (strncmp(v->v.s, prefix, len) != 0) return NULL;
    return v->v.s + len;
}
...
static void set_scope_explicit_typing(OfortScope *s) {
    ...
    memset(s->implicit_type_names, 0, sizeof(s->implicit_type_names));
}
...
static const char *implicit_type_name_for_name(OfortInterpreter *I, const char *name) {
    char c = name && name[0] ? (char)toupper((unsigned char)name[0]) : '\0';
    int idx = (c >= 'A' && c <= 'Z') ? c - 'A' : -1;
    OfortScope *s = I->current_scope;
    while (s && idx >= 0) {
        if (s->implicit_types[idx] == 'T' && s->implicit_type_names[idx][0])
            return s->implicit_type_names[idx];
        s = s->parent;
    }
    return "";
}
...
static OfortVar *set_var(OfortInterpreter *I, const char *name, OfortValue val) {
    ...
    if (has_implicit_type) {
        ...
        if (implicit_type == FVAL_DERIVED) {
            const char *type_name = implicit_type_name_for_name(I, name);
            if (val.type != FVAL_DERIVED ||
                (type_name[0] && !str_eq_nocase(val.v.dt.type_name, type_name))) {
        ...
        if (*p == '.') {
            ...
            if (*p == '.') {
                ...
                if (strcmp(dotword, "AND") == 0) { t->type = FTOK_AND; }
                    ...
                    t->type = FTOK_USER_OP;
                    snprintf(t->str_val, sizeof(t->str_val), ".%s.", dotword);
                }
                t->length = (int)(p - start);
                I->n_tokens++;
                continue;
...
static const char *token_type_name(OfortTokenType type) {
    switch (type) {
        ...
        case FTOK_USER_OP: return "user-defined operator";
...
static void describe_token(char *buf, size_t buf_size, const OfortToken *t) {
    ...
    if (t->type == FTOK_IDENT || t->type == FTOK_STRING_LIT || t->type == FTOK_USER_OP) {
...
static int token_can_be_name(OfortToken *t) {
    return t && (t->type == FTOK_IDENT || t->type == FTOK_IN ||
                 ...
                 t->type == FTOK_CASE || t->type == FTOK_CONTAINS ||
                 t->type == FTOK_DO || t->type == FTOK_ELSE ||
                 t->type == FTOK_ELSEIF || t->type == FTOK_IMPLICIT ||
                 t->type == FTOK_NONE || t->type == FTOK_THEN ||
                 t->type == FTOK_WHILE ||
...
static const char *token_name_text(OfortToken *t) {
    ...
    if (t->type == FTOK_CASE) return "case";
    if (t->type == FTOK_CONTAINS) return "contains";
    if (t->type == FTOK_DO) return "do";
    if (t->type == FTOK_ELSE) return "else";
    if (t->type == FTOK_ELSEIF) return "elseif";
    if (t->type == FTOK_IMPLICIT) return "implicit";
    if (t->type == FTOK_NONE) return "none";
    if (t->type == FTOK_THEN) return "then";
    if (t->type == FTOK_WHILE) return "while";
...
static int parse_operator_designator(OfortInterpreter *I, char *name, size_t name_size) {
    ...
    if (check(I, FTOK_USER_OP)) {
        ...
        advance(I);
        copy_cstr(name, name_size, "*");
    } else if (check(I, FTOK_SLASH)) {
        advance(I);
        copy_cstr(name, name_size, "/");
    } else if (check(I, FTOK_CONCAT)) {
        advance(I);
        copy_cstr(name, name_size, "//");
    } else if (check(I, FTOK_EQ)) {
        advance(I);
        copy_cstr(name, name_size, "==");
    } else if (check(I, FTOK_NEQ)) {
        advance(I);
        copy_cstr(name, name_size, "/=");
    } else if (check(I, FTOK_LT)) {
        advance(I);
        copy_cstr(name, name_size, "<");
    } else if (check(I, FTOK_LE)) {
        advance(I);
        copy_cstr(name, name_size, "<=");
    } else if (check(I, FTOK_GT)) {
        advance(I);
        copy_cstr(name, name_size, ">");
    } else if (check(I, FTOK_GE)) {
        advance(I);
        copy_cstr(name, name_size, ">=");
    } else {
        I->tok_pos = pos;
...
static OfortNode *parse_interface_block(OfortInterpreter *I) {
    ...
    n->line = it->line;
    n->n_params = 0;
    if (parse_operator_designator(I, n->name, sizeof(n->name))) {
        /* generic operator name was copied */
    ...
    if (t->type == FTOK_LBRACKET) {
        ...
        int cap = 0;
        {
            ...
            while (pos < I->n_tokens) {
                OfortTokenType tt = I->tokens[pos].type;
                if ((tt == FTOK_RBRACKET || tt == FTOK_NEWLINE || tt == FTOK_EOF) && depth == 0) break;
                if (tt == FTOK_LPAREN || tt == FTOK_LBRACKET) depth++;
                else if ((tt == FTOK_RPAREN || tt == FTOK_RBRACKET) && depth > 0) depth--;
                else if (tt == FTOK_DCOLON && depth == 0) {
                    has_type_spec = 1;
                    break;
                }
                pos++;
...
static OfortNode *parse_unary(OfortInterpreter *I) {
    if (check(I, FTOK_USER_OP)) {
        OfortToken *op = advance(I);
        OfortNode *arg = parse_unary(I);
        OfortNode *n = alloc_node(I, FND_FUNC_CALL);
        copy_cstr(n->name, sizeof(n->name), op->str_val);
        n->stmts = (OfortNode **)calloc(1, sizeof(OfortNode *));
        if (!n->stmts) ofort_error(I, "Out of memory parsing unary user-defined operator");
        n->stmts[0] = arg;
        n->n_stmts = 1;
        n->line = op->line;
        return n;
...
static OfortNode *parse_comparison(OfortInterpreter *I) {
    ...
    while (!expr_at_stop_token(I) &&
        ...
        if (op == FTOK_USER_OP) {
            OfortNode *n = alloc_node(I, FND_FUNC_CALL);
            copy_cstr(n->name, sizeof(n->name), op_token->str_val);
            n->stmts = (OfortNode **)calloc(2, sizeof(OfortNode *));
            if (!n->stmts) ofort_error(I, "Out of memory parsing user-defined operator");
            n->stmts[0] = left;
            n->stmts[1] = right;
            n->n_stmts = 2;
            n->line = left->line;
...
static OfortNode *parse_format_statement(OfortInterpreter *I, long long label) {
    ...
    if (check(I, FTOK_LPAREN)) {
        ...
        while (!check(I, FTOK_EOF) && depth > 0) {
            ...
            else if (tok->type == FTOK_RBRACKET) depth--;
        }
        if (depth > 0) ofort_error(I, "Unterminated FORMAT statement");
        {
        ...
        const char *start = ft->start + ft->length;
        ...
        {
            size_t len = (size_t)(end - start);
            if (len >= sizeof(n->format_str) - 2) len = sizeof(n->format_str) - 3;
            n->format_str[0] = '(';
            memcpy(n->format_str + 1, start, len);
            n->format_str[len + 1] = ')';
            n->format_str[len + 2] = '\0';
        }
        if (label > 0) register_format_label(I, (int)label, n->format_str);
    }
    return n;
        ...
        if (check_keyword_arg(I)) {
            ...
            if (str_eq_nocase(name, "unit")) {
                ...
                if (check(I, FTOK_STAR)) {
                    ...
                    if (peek_ahead(I, 1)->type == FTOK_CONCAT) {
                        n->children[1] = parse_expr(I);
            ...
            } else if (str_eq_nocase(name, "nml")) {
                if (!token_can_be_name(peek(I))) expect(I, FTOK_IDENT);
                copy_cstr(n->name, sizeof(n->name), token_name_text(advance(I)));
                saw_fmt = 1;
            ...
            if (positional > 0) saw_fmt = 1;
            if (positional > 0 && peek_ahead(I, 1)->type == FTOK_CONCAT) {
                n->children[1] = parse_expr(I);
                if (n->n_children < 2) n->n_children = 2;
            } else {
                copy_cstr(n->format_str, sizeof(n->format_str), peek(I)->str_val);
...
static OfortNode *parse_namelist_statement(OfortInterpreter *I) {
    OfortToken *nt = advance(I); /* NAMELIST */
    OfortNode *n = alloc_node(I, FND_NAMELIST);
    n->line = nt->line;
    expect(I, FTOK_SLASH);
    if (!token_can_be_name(peek(I))) expect(I, FTOK_IDENT);
    copy_cstr(n->name, sizeof(n->name), token_name_text(advance(I)));
    ...
    while (!check(I, FTOK_NEWLINE) && !check(I, FTOK_EOF)) {
        ...
        if (!token_can_be_name(peek(I))) expect(I, FTOK_IDENT);
        if (n->n_params >= OFORT_MAX_PARAMS) too_many_params_error(I, "NAMELIST variables");
        copy_cstr(n->param_names[n->n_params++], sizeof(n->param_names[0]),
                  token_name_text(advance(I)));
        if (check(I, FTOK_COMMA)) advance(I);
    }
    return n;
...
static OfortNode *parse_type_def(OfortInterpreter *I) {
    ...
    /* TYPE [ , EXTENDS(parent) ] :: name */
    char parent_type_name[64] = "";
    while (check(I, FTOK_COMMA)) {
        advance(I);
        if (token_ident_upper(peek(I), "EXTENDS")) {
            advance(I);
            expect(I, FTOK_LPAREN);
            if (!token_can_be_name(peek(I))) expect(I, FTOK_IDENT);
            copy_cstr(parent_type_name, sizeof(parent_type_name), token_name_text(advance(I)));
            expect(I, FTOK_RPAREN);
        } else {
            if (token_can_be_name(peek(I))) advance(I);
            if (check(I, FTOK_LPAREN)) {
    ...
    copy_cstr(n->parent_type_name, sizeof(n->parent_type_name), parent_type_name);
    n->line = tt->line;
    if (check(I, FTOK_LPAREN)) {
        advance(I);
        while (!check(I, FTOK_RPAREN) && !check(I, FTOK_EOF)) {
...
static OfortNode *parse_implicit_stmt(OfortInterpreter *I) {
    ...
    while (!check(I, FTOK_NEWLINE) && !check(I, FTOK_EOF)) {
        OfortValType vtype;
        char implicit_type_name[64];
        implicit_type_name[0] = '\0';
        ...
        while (!check(I, FTOK_RPAREN) && !check(I, FTOK_EOF)) {
            ...
            for (int i = lo; i <= hi; i++) {
                n->implicit_types[i] = vtype == FVAL_DERIVED ? 'T' : implicit_type_code(vtype);
                if (vtype == FVAL_CHARACTER) n->implicit_char_lens[i] = implicit_char_len;
                if (vtype == FVAL_DERIVED)
                    copy_cstr(n->implicit_type_names[i], sizeof(n->implicit_type_names[i]), implicit_type_name);
                else
                    n->implicit_type_names[i][0] = '\0';
            }
            if (check(I, FTOK_COMMA)) advance(I);
            else break;
        }
...
static OfortNode *parse_statement(OfortInterpreter *I) {
    ...
    if (token_ident_upper(t, "ABSTRACT") && token_ident_upper(peek_ahead(I, 1), "INTERFACE")) {
        advance(I); /* ABSTRACT */
    ...
    if (t->type == FTOK_INT_LIT) {
        ...
        advance(I);
        t = peek(I);
        if (token_ident_upper(t, "FORMAT") && peek_ahead(I, 1)->type != FTOK_ASSIGN) {
    ...
    if (peek_ahead(I, 1)->type == FTOK_ASSIGN && token_can_be_name(t)) {
        leave_spec_section(I);
        return parse_keyword_assignment_statement(I);
    }

    if (token_ident_upper(t, "NAMELIST")) {
        return parse_namelist_statement(I);
    }
    ...
    if (t->type == FTOK_USE) {
        ...
        while (!check(I, FTOK_NEWLINE) && !check(I, FTOK_EOF)) {
            ...
            } else if (token_ident_upper(peek(I), "OPERATOR")) {
                char local_op[256];
                char remote_op[256];
                if (!parse_operator_designator(I, local_op, sizeof(local_op))) {
                    advance(I);
                    continue;
                }
                copy_cstr(remote_op, sizeof(remote_op), local_op);
                if (check(I, FTOK_POINTER_ASSIGN)) {
                    advance(I);
                    if (!parse_operator_designator(I, remote_op, sizeof(remote_op))) {
                        if (!token_can_be_name(peek(I))) expect(I, FTOK_IDENT);
                        copy_cstr(remote_op, sizeof(remote_op), token_name_text(advance(I)));
                    }
                }
                if (n->n_params >= OFORT_MAX_PARAMS) too_many_params_error(I, "USE ONLY names");
                copy_cstr(n->param_names[n->n_params], sizeof(n->param_names[0]), local_op);
                copy_cstr(n->binding_proc_names[n->n_params], sizeof(n->binding_proc_names[0]), remote_op);
                n->n_params++;
            } else if (token_can_be_name(peek(I))) {
    ...
    if (t->type == FTOK_TYPE) {
        ...
        OfortToken *next = peek_ahead(I, 1);
        if (next->type == FTOK_COMMA || next->type == FTOK_DCOLON || token_can_be_name(next)) {
...
static OfortNode *parse_block_until_where_part(OfortInterpreter *I) {
    OfortNode *block = alloc_node(I, FND_BLOCK);
    int cap = 0;
    block->stmts = NULL;
    block->n_stmts = 0;
    ...
    while (peek(I)->type != FTOK_EOF &&
           !check_end(I, "WHERE") &&
           !token_ident_upper(peek(I), "ELSEWHERE") &&
           !(peek(I)->type == FTOK_ELSE && token_ident_upper(peek_ahead(I, 1), "WHERE"))) {
        skip_newlines(I);
        if (peek(I)->type == FTOK_EOF ||
            check_end(I, "WHERE") ||
            token_ident_upper(peek(I), "ELSEWHERE") ||
            (peek(I)->type == FTOK_ELSE && token_ident_upper(peek_ahead(I, 1), "WHERE"))) break;
        int before_pos = I->tok_pos;
        OfortNode *s = parse_statement(I);
        if (s) {
            if (block->n_stmts >= cap) {
                cap = cap ? cap * 2 : 8;
                block->stmts = (OfortNode **)realloc(block->stmts, sizeof(OfortNode *) * (size_t)cap);
                if (!block->stmts) ofort_error(I, "Out of memory parsing WHERE");
            }
            block->stmts[block->n_stmts++] = s;
        }
        if (I->tok_pos == before_pos && !check(I, FTOK_EOF)) skip_to_next_line(I);
        skip_newlines(I);
    }
    return block;
}

static int check_elsewhere_stmt(OfortInterpreter *I) {
    return token_ident_upper(peek(I), "ELSEWHERE") ||
           (peek(I)->type == FTOK_ELSE && token_ident_upper(peek_ahead(I, 1), "WHERE"));
}

static void consume_elsewhere_stmt(OfortInterpreter *I) {
    if (token_ident_upper(peek(I), "ELSEWHERE")) {
        advance(I);
    } else {
        expect(I, FTOK_ELSE);
...
static OfortNode *parse_where_statement(OfortInterpreter *I) {
    ...
    if (!check(I, FTOK_NEWLINE) && !check(I, FTOK_EOF)) {
        OfortNode *block = alloc_node(I, FND_BLOCK);
        block->stmts = (OfortNode **)calloc(1, sizeof(OfortNode *));
        if (!block->stmts) ofort_error(I, "Out of memory parsing WHERE");
        block->stmts[0] = parse_statement(I);
        block->n_stmts = block->stmts[0] ? 1 : 0;
        n->children[1] = block;
        return n;
    }
...
static OfortValue default_derived_value(OfortInterpreter *I, const char *type_name) {
    ...
    for (int i = 0; i < td->n_fields; i++) {
        ...
        } else if (td->field_is_pointer[i] && field_type == FVAL_DERIVED) {
            v.v.dt.fields[i] = make_void_val();
...
static void assign_character_substring(OfortInterpreter *I, OfortVar *var, OfortNode *lhs, OfortValue *rhs) {
    int len;
    OfortSubscriptRange range;
    char rbuf[OFORT_MAX_STRLEN];
    int src_len;

    if (!var || var->val.type != FVAL_CHARACTER || !var->val.v.s)
        ofort_error(I, "Substring assignment target is not character");
    if (!lhs || lhs->n_stmts != 1)
    ...
    for (int i = 0; i < range.count; i++) {
        var->val.v.s[range.start - 1 + i] = i < src_len ? rbuf[i] : ' ';
    }
}
    ...
    if (dim < 0) {
        ...
        if (I->where_mask) {
            ...
            if (I->where_mask->type == FVAL_ARRAY) {
                if (mask_index < I->where_mask->v.arr.len) {
                    OfortValue mv = array_element_value(I->where_mask, mask_index);
                    take = val_to_logical(mv);
                    free_value(&mv);
                }
            } else {
                take = val_to_logical(*I->where_mask);
            }
        ...
        if (rhs->type == FVAL_ARRAY) {
            if (*rhs_index >= rhs->v.arr.len)
                ofort_error(I, "Array assignment shape mismatch assigning array section (rhs index %d, rhs len %d)",
                            *rhs_index, rhs->v.arr.len);
...
static void assign_array_ref(OfortInterpreter *I, OfortVar *var, OfortNode *lhs, OfortValue *rhs) {
    ...
    if (rhs->type == FVAL_ARRAY && rhs->v.arr.len != selected)
        ofort_error(I, "Array assignment shape mismatch assigning to '%s' (selected %d, rhs %d, nargs %d, has_section %d)",
                    var->name, selected, rhs->v.arr.len, nargs, has_section);
...
static void ensure_data_target_declared(OfortInterpreter *I, OfortNode *target) {
    if (!target) return;
    if (target->type == FND_IDENT && !find_var(I, target->name)) {
        int has_type = 0;
        OfortValType type;
        int char_len = 0;
        if (current_scope_has_implicit_none(I))
        ...
        if (type == FVAL_DERIVED) {
            ...
            declare_var(I, target->name, default_derived_value(I, type_name));
        } else {
        ...
        copy_cstr(ident.name, sizeof(ident.name), target->name);
        ensure_data_target_declared(I, &ident);
    } else if (target->type == FND_MEMBER && target->children[0]) {
        ensure_data_target_declared(I, target->children[0]);
    }
}
...
static OfortNode *data_node_from_value(OfortInterpreter *I, OfortValue *value) {
    ...
    case FVAL_DERIVED:
        n = alloc_node(I, FND_FUNC_CALL);
        copy_cstr(n->name, sizeof(n->name), value->v.dt.type_name);
        n->line = 0;
        n->stmts = NULL;
        n->n_stmts = 0;
        {
            ...
            for (int i = 0; i < value->v.dt.n_fields; i++) {
                if (cap <= n->n_stmts) {
                    cap = cap ? cap * 2 : 4;
                    n->stmts = (OfortNode **)realloc(n->stmts, sizeof(OfortNode *) * (size_t)cap);
                    if (!n->stmts) ofort_error(I, "Out of memory building DATA derived node");
                }
                n->stmts[n->n_stmts++] = data_node_from_value(I, &value->v.dt.fields[i]);
            }
        }
        return n;
    case FVAL_VOID:
        n = alloc_node(I, FND_FUNC_CALL);
        copy_cstr(n->name, sizeof(n->name), "null");
        n->line = 0;
        n->stmts = NULL;
        n->n_stmts = 0;
        return n;
...
static int string_equal_fortran(const char *a, const char *b) {
    ...
    return na == nb && strncmp(a ? a : "", b ? b : "", na) == 0;
}

static int string_compare_fortran(const char *a, const char *b) {
    size_t na = a ? strlen(a) : 0;
    size_t nb = b ? strlen(b) : 0;
    size_t n = na > nb ? na : nb;
    for (size_t i = 0; i < n; i++) {
        unsigned char ca = i < na ? (unsigned char)a[i] : (unsigned char)' ';
        unsigned char cb = i < nb ? (unsigned char)b[i] : (unsigned char)' ';
        if (ca < cb) return -1;
        if (ca > cb) return 1;
    }
    return 0;
...
static int values_equal_fortran(OfortInterpreter *I, OfortValue a, OfortValue b) {
    if (a.type == FVAL_CHARACTER || b.type == FVAL_CHARACTER) {
        char abuf[OFORT_MAX_STRLEN], bbuf[OFORT_MAX_STRLEN];
        value_to_string(I, a, abuf, sizeof(abuf));
        value_to_string(I, b, bbuf, sizeof(bbuf));
        return string_equal_fortran(abuf, bbuf);
    }
    if (a.type == FVAL_LOGICAL || b.type == FVAL_LOGICAL)
        return val_to_logical(a) == val_to_logical(b);
    if (a.type == FVAL_INTEGER && b.type == FVAL_INTEGER)
        return val_to_int(a) == val_to_int(b);
    return val_to_real(a) == val_to_real(b);
}

static int values_compare_fortran(OfortInterpreter *I, OfortValue a, OfortValue b) {
    if (a.type == FVAL_CHARACTER || b.type == FVAL_CHARACTER) {
        char abuf[OFORT_MAX_STRLEN], bbuf[OFORT_MAX_STRLEN];
        value_to_string(I, a, abuf, sizeof(abuf));
        value_to_string(I, b, bbuf, sizeof(bbuf));
        return string_compare_fortran(abuf, bbuf);
    }
    {
        double av = val_to_real(a);
        double bv = val_to_real(b);
        if (av < bv) return -1;
        if (av > bv) return 1;
        return 0;
    }
...
static OfortValue eval_in_operator(OfortInterpreter *I, OfortValue item, OfortValue collection) {
    int found = 0;
    if (collection.type != FVAL_ARRAY) {
        found = values_equal_fortran(I, item, collection);
    } else {
        for (int i = 0; i < collection.v.arr.len; i++) {
            OfortValue elem = array_element_value(&collection, i);
            found = values_equal_fortran(I, item, elem);
            free_value(&elem);
            if (found) break;
        }
    }
    return make_logical(found);
}
...
static void copy_trimmed_fortran_identifier(const char *src, char *dst, size_t dst_size) {
    size_t len;
    if (!src || dst_size == 0) return;
    copy_cstr(dst, dst_size, src);
    len = strlen(dst);
    while (len > 0 && isspace((unsigned char)dst[len - 1])) dst[--len] = '\0';
    while (len > 0 && isspace((unsigned char)dst[0])) memmove(dst, dst + 1, strlen(dst));
    while ((signed)len > 0 && isspace((unsigned char)dst[len - 1])) dst[--len] = '\0';
    for (size_t i = 0; i < len && i + 1 < dst_size; i++) {
        dst[i] = (char)tolower((unsigned char)dst[i]);
    }
    if (len >= dst_size) dst[dst_size - 1] = '\0';
}

static void write_direct_record_from_node(OfortInterpreter *I, OfortUnitFile *entry,
                                         int rec, const char *fmt, OfortValue *vals, int nvals,
                                         int *status_ptr) {
    FILE *fp = NULL;
    long long offset;
    char record[OFORT_MAX_OUTPUT];
    size_t record_len;
    int saved_len;
    char saved_out[OFORT_MAX_OUTPUT];

    if (!entry || rec <= 0 || entry->recl <= 0) {
    ...
    if (!fp) ofort_error(I, "Cannot open '%s' for direct write", entry->path);
    copy_cstr(saved_out, sizeof(saved_out), I->output);
    saved_len = I->out_len;
    I->out_len = 0;
    I->output[0] = '\0';
    format_output(I, fmt, vals, nvals);
    copy_cstr(record, sizeof(record), I->output);
    I->out_len = saved_len;
    copy_cstr(I->output, sizeof(I->output), saved_out);
    memset(saved_out, 0, sizeof(saved_out));
    record_len = strlen(record);
    if (record_len >= (size_t)entry->recl) {
        record[entry->recl] = '\0';
    } else {
        size_t pad_len = (size_t)entry->recl - record_len;
        memset(record + record_len, ' ', pad_len);
        record[entry->recl] = '\0';
    }
    offset = (long long)(rec - 1) * (long long)entry->recl;
    if (fseek(fp, offset, SEEK_SET) != 0) {
        fclose(fp);
        ofort_error(I, "Failed to seek record %d in '%s'", rec, entry->path);
    }
    if ((size_t)fwrite(record, 1, (size_t)entry->recl, fp) != (size_t)entry->recl) {
        fclose(fp);
        ofort_error(I, "Failed to write record %d in '%s'", rec, entry->path);
    }
    fflush(fp);
    fclose(fp);
...
static int read_direct_record_as_string(OfortInterpreter *I, OfortUnitFile *entry, int rec, char *buf, int bufsize) {
    ...
    int status = 0;
    (void)bufsize;
    char tmp[OFORT_MAX_OUTPUT];

    if (!entry || rec <= 0 || entry->recl <= 0) return 1;
    if (entry->recl <= 0 || entry->recl >= (int)sizeof(tmp)) return 1;
    fp = fopen(entry->path, "rb");
    if (!fp) return 1;
    offset = (long long)(rec - 1) * (long long)entry->recl;
    if (fseek(fp, offset, SEEK_SET) != 0) {
        fclose(fp);
    ...
    nread = fread(tmp, 1, (size_t)entry->recl, fp);
    fclose(fp);
    if (nread == 0) return 1;
    if (nread < (size_t)entry->recl) {
        memset(tmp + nread, ' ', (size_t)entry->recl - nread);
        nread = (size_t)entry->recl;
    }
    if (nread >= sizeof(tmp)) nread = sizeof(tmp) - 1;
...
static int read_direct_from_file(OfortInterpreter *I, OfortUnitFile *entry, OfortNode *n, int rec) {
    char recbuf[OFORT_MAX_OUTPUT];
    int status = read_direct_record_as_string(I, entry, rec, recbuf, sizeof(recbuf));
    if (format_is_character_line_read(n->format_str) && n->n_stmts > 0) {
        if (status == 0) {
            assign_line_to_character_target(I, n->stmts[0], recbuf);
        }
        return status;
    }
    if (status == 0) {
...
static void namelist_emit(OfortInterpreter *I, FILE *fp, const char *text) {
    if (fp) fputs(text, fp);
    else out_append(I, text);
}

static void namelist_emitf(OfortInterpreter *I, FILE *fp, const char *fmt, ...) {
    char buf[2048];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(buf, sizeof(buf), fmt, ap);
    va_end(ap);
    namelist_emit(I, fp, buf);
}

static void namelist_write_value(OfortInterpreter *I, FILE *fp, const OfortValue *v) {
    if (!v) {
        namelist_emit(I, fp, " ");
        return;
    }
    switch (v->type) {
    case FVAL_INTEGER:
        namelist_emitf(I, fp, " %lld", v->v.i);
        break;
    case FVAL_REAL:
    case FVAL_DOUBLE:
        namelist_emitf(I, fp, " %.17g", v->v.r);
        break;
    case FVAL_LOGICAL:
        namelist_emit(I, fp, v->v.b ? " T" : " F");
        break;
    case FVAL_CHARACTER:
        namelist_emitf(I, fp, " \"%s\"", v->v.s ? v->v.s : "");
        break;
    default:
        namelist_emit(I, fp, " ");
        break;
...
static void write_namelist_to_stream(OfortInterpreter *I, const char *group_name, FILE *fp) {
    OfortNamelist *nl = find_namelist(I, group_name);
    if (!nl) ofort_error(I, "Undefined NAMELIST group '%s'", group_name);
    namelist_emitf(I, fp, "&%s\n", nl->name);
    for (int i = 0; i < nl->n_vars; i++) {
        OfortVar *v = find_var(I, nl->var_names[i]);
        if (!v) ofort_error(I, "Undefined NAMELIST variable '%s'", nl->var_names[i]);
        namelist_emitf(I, fp, " %s =", v->name);
        if (v->val.type == FVAL_ARRAY) {
            for (int j = 0; j < v->val.v.arr.len; j++) {
                OfortValue elem = array_element_value(&v->val, j);
                namelist_write_value(I, fp, &elem);
                free_value(&elem);
                if (j + 1 < v->val.v.arr.len) namelist_emit(I, fp, ",");
            }
        } else {
            namelist_write_value(I, fp, &v->val);
        }
        namelist_emit(I, fp, ",\n");
    }
    namelist_emit(I, fp, "/\n");
...
static const char *namelist_skip_space_commas(const char *p) {
    while (*p && (isspace((unsigned char)*p) || *p == ',')) p++;
    return p;
}

static int namelist_read_word(const char **pp, char *buf, int buf_size) {
    const char *p = *pp;
    int n = 0;
    p = namelist_skip_space_commas(p);
    if (!isalpha((unsigned char)*p) && *p != '_') return 0;
    while ((isalnum((unsigned char)*p) || *p == '_') && n < buf_size - 1) {
        buf[n++] = *p++;
    }
    while (isalnum((unsigned char)*p) || *p == '_') p++;
    buf[n] = '\0';
...
static int namelist_next_is_assignment(const char *p) {
    char name[256];
    p = namelist_skip_space_commas(p);
    if (*p == '/' || *p == '&' || *p == '$' || !*p) return 1;
    if (!namelist_read_word(&p, name, sizeof(name))) return 0;
    while (isspace((unsigned char)*p)) p++;
    return *p == '=';
}

static int namelist_read_value_token(const char **pp, char *buf, int buf_size) {
    const char *p = namelist_skip_space_commas(*pp);
    int n = 0;
    if (*p == '/' || !*p) return 0;
    if (*p == '\'' || *p == '"') {
        char quote = *p++;
        while (*p && *p != quote) {
            if (n < buf_size - 1) buf[n++] = *p;
            p++;
        }
        if (*p == quote) p++;
    } else {
...
static void namelist_assign_scalar(OfortInterpreter *I, OfortVar *v, const char *tok) {
    OfortValue tmp = default_value(v->val.type, v->char_len > 0 ? v->char_len : 1);
    assign_token_to_value(&tmp, tok);
    set_var(I, v->name, tmp);
}

static void namelist_assign_array_element(OfortValue *arr, int index, const char *tok) {
    OfortValue tmp = default_value(arr->v.arr.elem_type, 1);
    assign_token_to_value(&tmp, tok);
    if (assign_packed_array_element(arr, index, tmp)) {
        free_value(&tmp);
    } else if (arr->v.arr.data && index >= 0 && index < arr->v.arr.len) {
...
static int read_namelist_from_text(OfortInterpreter *I, const char *group_name, const char *text) {
    OfortNamelist *nl = find_namelist(I, group_name);
    const char *p = text ? text : "";
    char word[256];
    if (!nl) ofort_error(I, "Undefined NAMELIST group '%s'", group_name);
    while (*p) {
        if (*p == '&' || *p == '$') {
            p++;
            if (namelist_read_word(&p, word, sizeof(word)) && str_eq_nocase(word, nl->name)) break;
        } else {
            p++;
        }
    }
    ...
    for (;;) {
        ...
        p = namelist_skip_space_commas(p);
        if (*p == '/' || !*p) break;
        if (!namelist_read_word(&p, word, sizeof(word))) {
            p++;
            continue;
        }
        while (isspace((unsigned char)*p)) p++;
        ...
        p++;
        v = find_var(I, word);
        if (!v) ofort_error(I, "Undefined NAMELIST variable '%s'", word);
        if (v->val.type == FVAL_ARRAY) {
            int index = 0;
            while (index < v->val.v.arr.len && namelist_read_value_token(&p, value, sizeof(value))) {
                namelist_assign_array_element(&v->val, index++, value);
                if (namelist_next_is_assignment(p)) break;
            }
        } else if (namelist_read_value_token(&p, value, sizeof(value))) {
            namelist_assign_scalar(I, v, value);
        }
    }
    return 0;
}

static int read_namelist_from_file(OfortInterpreter *I, OfortUnitFile *entry, const char *group_name) {
    ...
    char *text;
    int status;
    if (!fp) ofort_error(I, "Cannot open '%s' for namelist reading", entry->path);
    fseek(fp, entry->stream_pos, SEEK_SET);
    start = ftell(fp);
    fseek(fp, 0, SEEK_END);
    size = ftell(fp) - start;
    ...
    if (!text) {
        fclose(fp);
        ofort_error(I, "Out of memory reading NAMELIST");
    }
    nread = fread(text, 1, (size_t)size, fp);
    text[nread] = '\0';
    entry->stream_pos = (int)ftell(fp);
    fclose(fp);
    status = read_namelist_from_text(I, group_name, text);
    free(text);
    return status;
}
...
static void render_write_to_string(OfortInterpreter *I, const char *fmt,
    ...
    copy_cstr(buf, (size_t)bufsize, I->output);
    I->out_len = saved_len;
...

    {
        ...
        if (val.type == FVAL_ARRAY) {
            for (int i = 0; i < val.v.arr.len; i++) {
                append_io_value(I, vals, nvals, cap, array_element_value(&val, i));
            }
            free_value(&val);
        } else {
            append_io_value(I, vals, nvals, cap, val);
        }
    }
}
...
static OfortValue eval_node(OfortInterpreter *I, OfortNode *n) {
    ...
    case FND_IDENT: {
        ...
        if (!v) {
            ...
            if (!current_scope_has_implicit_none(I)) {
                int has_implicit_type = 0;
                OfortValType implicit_type = implicit_type_for_name(I, n->name, &has_implicit_type);
                if (has_implicit_type && implicit_type == FVAL_DERIVED) {
                    const char *type_name = implicit_type_name_for_name(I, n->name);
                    if (type_name[0]) {
                        OfortValue val = default_derived_value(I, type_name);
                        v = declare_var(I, n->name, val);
    ...
    case FND_AND: {
        ...
        if (left.type == FVAL_ARRAY || right.type == FVAL_ARRAY) {
            ...
            int len = array_arg->v.arr.len;
            if (left.type == FVAL_ARRAY && right.type == FVAL_ARRAY && left.v.arr.len != right.v.arr.len)
                ofort_error(I, "Array size mismatch in logical operation");
            for (int i = 0; i < len; i++) {
                ...
                free_value(&result_array.v.arr.data[i]);
                result_array.v.arr.data[i] = make_logical(val_to_logical(lv) && val_to_logical(rv));
                free_value(&lv);
                free_value(&rv);
            }
            free_value(&left); free_value(&right);
    ...
    case FND_FUNC_CALL: {
        ...
        if (var && var->val.type == FVAL_CHARACTER) {
            const char *proc_name = procedure_ref_name(&var->val);
            if (proc_name) {
                copy_cstr(procedure_call_name, sizeof(procedure_call_name), proc_name);
                {
                    size_t len = strlen(procedure_call_name);
                    while (len > 0 && isspace((unsigned char)procedure_call_name[len - 1]))
                        procedure_call_name[--len] = '\0';
                }
            } else {
...
static OfortNamelist *find_namelist(OfortInterpreter *I, const char *name) {
    for (int i = 0; i < I->n_namelists; i++) {
        if (str_eq_nocase(I->namelists[i].name, name)) return &I->namelists[i];
    }
    return NULL;
}
...
static int type_extends_or_same(OfortInterpreter *I, const char *actual_name,
                                const char *expected_name) {
    if (!actual_name || !actual_name[0] || !expected_name || !expected_name[0])
        return 0;
    if (str_eq_nocase(actual_name, expected_name)) return 1;
    OfortTypeDef *td = find_type_def(I, actual_name);
    int depth = 0;
    while (td && td->parent_type_name[0] && depth++ < 64) {
        if (str_eq_nocase(td->parent_type_name, expected_name)) return 1;
        td = find_type_def(I, td->parent_type_name);
    }
    return 0;
}
...
static void register_namelist(OfortInterpreter *I, OfortNode *n) {
    ...
    if (!nl) {
        ...
        copy_cstr(nl->name, sizeof(nl->name), n->name);
    }
    nl->n_vars = 0;
    for (int i = 0; i < n->n_params; i++) {
        if (nl->n_vars >= OFORT_MAX_PARAMS) too_many_params_error(I, "NAMELIST variables");
        copy_cstr(nl->var_names[nl->n_vars++], sizeof(nl->var_names[0]), n->param_names[i]);
    }
}
...
static const char *io_statement_name(OfortNodeType type) {
    switch (type) {
        ...
        case FND_NAMELIST: return "NAMELIST";
...
static void exec_node(OfortInterpreter *I, OfortNode *n) {
    ...
    case FND_WHERE: {
        OfortValue mask = eval_node(I, n->children[0]);
    ...
    case FND_USE: {
        ...
        for (int i = 0; i < n->n_params; i++) {
            const char *local = n->param_names[i];
            const char *remote = n->binding_proc_names[i][0] ? n->binding_proc_names[i] : local;
            OfortTypeDef *src_type = find_type_def(I, remote);
            if (src_type && !find_type_def(I, local)) {
                if (I->n_type_defs >= 64) ofort_error(I, "Too many type definitions");
                I->type_defs[I->n_type_defs] = *src_type;
                copy_cstr(I->type_defs[I->n_type_defs].name,
                          sizeof(I->type_defs[I->n_type_defs].name), local);
                I->n_type_defs++;
            }
    ...
    case FND_TYPE_DEF: {
        ...
        copy_cstr(td->parent_type_name, sizeof(td->parent_type_name), n->parent_type_name);
        td->n_fields = 0;
        td->n_bindings = 0;
        if (td->parent_type_name[0]) {
            OfortTypeDef *parent = find_type_def(I, td->parent_type_name);
            if (!parent)
                ofort_error(I, "Unknown parent type '%s' in EXTENDS for type '%s'",
                            td->parent_type_name, td->name);
            td->n_fields = parent->n_fields;
            for (int i = 0; i < parent->n_fields; i++) {
                copy_cstr(td->field_names[i], sizeof(td->field_names[i]), parent->field_names[i]);
                copy_cstr(td->field_type_names[i], sizeof(td->field_type_names[i]), parent->field_type_names[i]);
                td->field_types[i] = parent->field_types[i];
            ...
            td->n_bindings = parent->n_bindings;
            for (int i = 0; i < parent->n_bindings; i++) {
        ...
        for (int i = 0; i < n->n_params && i < OFORT_MAX_PARAMS; i++) {
            int binding_index = -1;
            for (int j = 0; j < td->n_bindings; j++) {
                if (str_eq_nocase(td->binding_names[j], n->param_names[i])) {
                    binding_index = j;
                    break;
                }
            }
            if (binding_index < 0) {
                if (td->n_bindings >= OFORT_MAX_PARAMS) too_many_params_error(I, "type-bound procedures");
                binding_index = td->n_bindings++;
            }
            copy_cstr(td->binding_names[binding_index],
                      sizeof(td->binding_names[binding_index]),
                      n->param_names[i]);
            copy_cstr(td->binding_proc_names[binding_index],
                      sizeof(td->binding_proc_names[binding_index]),
                ...
                for (int j = 0; j < s->n_stmts; j++) {
                    ...
                    if ((d->type == FND_VARDECL || d->type == FND_PARAMDECL) && td->n_fields < OFORT_MAX_FIELDS) {
                        ...
                        if (d->is_pointer && td->field_init_exprs[td->n_fields]) {
                            OfortValue init = eval_node(I, td->field_init_exprs[td->n_fields]);
                            td->field_init_exprs[td->n_fields] = data_node_from_value(I, &init);
                            free_value(&init);
                        }
    ...
    case FND_IMPLICIT_NONE:
        if (I->current_scope) {
            if (n->bool_val) {
                ...
                memset(I->current_scope->implicit_type_names, 0, sizeof(I->current_scope->implicit_type_names));
            } else {
                ...
                for (int i = 0; i < 26; i++) {
                    ...
                    if (n->implicit_types[i] == 'T')
                        copy_cstr(I->current_scope->implicit_type_names[i],
                                  sizeof(I->current_scope->implicit_type_names[i]),
                                  n->implicit_type_names[i]);
                    else if (n->implicit_types[i])
                        I->current_scope->implicit_type_names[i][0] = '\0';
                }
            }
        }
        break;
    ...
    case FND_PARAMDECL: {
        ...
        if (n->val_type == FVAL_VOID && n->n_dims > 0) {
            if (existing && existing == existing_current) {
                ...
                if (elem_type == FVAL_VOID) {
                    ...
                    if (elem_type == FVAL_DERIVED) {
                        const char *type_name = implicit_type_name_for_name(I, n->name);
                        if (type_name[0]) copy_cstr(n->str_val, sizeof(n->str_val), type_name);
                    }
                }
        ...
        if (existing && existing == existing_current && n->n_dims > 0) {
            int has_assumed_shape = 0;
            for (int i = 0; i < n->n_dims; i++) {
                if (n->dims[i] == 0) { has_assumed_shape = 1; break; }
            }
                ...
                if (rhs.type == FVAL_ARRAY) {
                    ...
                    for (int i = 0; i < v->val.v.arr.len; i++) {
                        if (I->where_mask) {
                            int take = 0;
                            if (I->where_mask->type == FVAL_ARRAY) {
                                if (i < I->where_mask->v.arr.len) {
                                    OfortValue mv = array_element_value(I->where_mask, i);
                                    take = val_to_logical(mv);
                                    free_value(&mv);
                                }
                            } else {
        ...
        if (n->name[0]) {
            ...
            if (target && target->val.type == FVAL_CHARACTER) {
                ...
                if (!entry && unit == 6) {
                    ...
                    if (entry->is_direct) {
                        if (has_rec) {
                            write_direct_record_from_node(I, entry, rec_no, fmt, vals, nvals, &status);
                        } else {
                            status = 1;
                        }
                    } else if (n->bool_val) {
        ...
        if (n->children[0]) {
            ...
            if (uv.type == FVAL_CHARACTER) {
                ...
                if (unit == 5) {
                    ...
                    if (entry->is_direct) {
                        if (has_rec) status = read_direct_from_file(I, entry, n, rec_no);
                        else status = 1;
                    } else if (n->bool_val) {
                        status = 0;
                        read_values_from_stream_file(I, entry, n);
                    } else {
    ...
    case FND_OPEN: {
        ...
        if (n->children[5]) {
            ...
            if (av.type == FVAL_CHARACTER) {
                copy_trimmed_fortran_identifier(av.v.s ? av.v.s : "", access_opt, sizeof(access_opt));
                if (strcmp(access_opt, "direct") == 0) is_direct = 1;
            }
            free_value(&av);
        }
        if (n->children[6]) {
            OfortValue fv2 = eval_node(I, n->children[6]);
            if (fv2.type == FVAL_CHARACTER) {
                copy_trimmed_fortran_identifier(fv2.v.s ? fv2.v.s : "", form_opt, sizeof(form_opt));
                if (strcmp(form_opt, "unformatted") == 0) is_formatted = 0;
            }
            free_value(&fv2);
        }
        if (n->children[7]) {
            OfortValue rv = eval_node(I, n->children[7]);
...
static OfortValue call_intrinsic(OfortInterpreter *I, const char *name, OfortValue *args, int nargs,
    ...
    if (strcmp(upper, "MERGE_BITS") == 0 ||
        ...
        int is_merge = strcmp(upper, "MERGE_BITS") == 0;
        int needed = is_merge ? 3 : 2;
        int idxs[3];
        OfortValue *shape_arg = NULL;
        int result_type = is_merge ? FVAL_INTEGER : FVAL_LOGICAL;
        if (i_idx < 0) i_idx = 0;
        if (j_idx < 0) j_idx = 1;
        if (is_merge && mask_idx < 0) mask_idx = 2;
        if (nargs < needed || i_idx >= nargs || j_idx >= nargs || (is_merge && mask_idx >= nargs))
            ofort_error(I, "%s requires %d arguments", upper, needed);
        idxs[0] = i_idx;
        idxs[1] = j_idx;
        idxs[2] = mask_idx;
        for (int ai = 0; ai < needed; ai++) {
            OfortValue *arg = &args[idxs[ai]];
            if (arg->type == FVAL_ARRAY) {
                if (!shape_arg) {
                    shape_arg = arg;
        ...
        if (shape_arg) {
            OfortValue result = make_array((OfortValType)result_type, shape_arg->v.arr.dims, shape_arg->v.arr.n_dims);
            for (int ri = 0; ri < result.v.arr.len; ri++) {
                OfortValue av = args[i_idx].type == FVAL_ARRAY ? array_element_value(&args[i_idx], ri) : copy_value(args[i_idx]);
                OfortValue bv = args[j_idx].type == FVAL_ARRAY ? array_element_value(&args[j_idx], ri) : copy_value(args[j_idx]);
                OfortValue mv = make_void_val();
                int kind, bits;
                unsigned long long bitmask;
                if (av.type != FVAL_INTEGER || bv.type != FVAL_INTEGER)
                    ofort_error(I, "%s requires integer arguments", upper);
                kind = av.kind ? av.kind : 4;
                bits = integer_kind_bits(kind);
                bitmask = bits >= 64 ? ~0ULL : ((1ULL << (unsigned int)bits) - 1ULL);
                free_value(&result.v.arr.data[ri]);
                if (is_merge) {
                    unsigned long long ia, ja, ma, out;
                    mv = args[mask_idx].type == FVAL_ARRAY ? array_element_value(&args[mask_idx], ri) : copy_value(args[mask_idx]);
                    if (mv.type != FVAL_INTEGER) ofort_error(I, "MERGE_BITS requires integer arguments");
                    ia = ((unsigned long long)av.v.i) & bitmask;
                    ja = ((unsigned long long)bv.v.i) & bitmask;
                    ma = ((unsigned long long)mv.v.i) & bitmask;
                    out = (ia & ma) | (ja & (~ma & bitmask));
                    result.v.arr.data[ri] = make_integer_kind(signed_mask_value(out, bits), kind);
                    free_value(&mv);
                } else {
                    unsigned long long ia = ((unsigned long long)av.v.i) & bitmask;
                    unsigned long long ja = ((unsigned long long)bv.v.i) & bitmask;
                    int ok = strcmp(upper, "BGE") == 0 ? ia >= ja :
                             strcmp(upper, "BGT") == 0 ? ia > ja :
                             strcmp(upper, "BLE") == 0 ? ia <= ja : ia < ja;
            ...
            int kind, bits;
            unsigned long long bitmask;
            if (args[i_idx].type != FVAL_INTEGER || args[j_idx].type != FVAL_INTEGER)
                ofort_error(I, "%s requires integer arguments", upper);
            kind = args[i_idx].kind ? args[i_idx].kind : 4;
            bits = integer_kind_bits(kind);
            bitmask = bits >= 64 ? ~0ULL : ((1ULL << (unsigned int)bits) - 1ULL);
            if (is_merge) {
                unsigned long long ia, ja, ma, out;
                if (args[mask_idx].type != FVAL_INTEGER) ofort_error(I, "MERGE_BITS requires integer arguments");
                ia = ((unsigned long long)args[i_idx].v.i) & bitmask;
                ja = ((unsigned long long)args[j_idx].v.i) & bitmask;
                ma = ((unsigned long long)args[mask_idx].v.i) & bitmask;
                out = (ia & ma) | (ja & (~ma & bitmask));
                return make_integer_kind(signed_mask_value(out, bits), kind);
            } else {
                unsigned long long ia = ((unsigned long long)args[i_idx].v.i) & bitmask;
                unsigned long long ja = ((unsigned long long)args[j_idx].v.i) & bitmask;
                int ok = strcmp(upper, "BGE") == 0 ? ia >= ja :
                         strcmp(upper, "BGT") == 0 ? ia > ja :
                         strcmp(upper, "BLE") == 0 ? ia <= ja : ia < ja;
    ...
    if ((strcmp(upper, "MOD") == 0 || strcmp(upper, "MODULO") == 0 ||
        ...
        if (strcmp(upper, "NEAREST") == 0 || strcmp(upper, "SCALE") == 0 ||
            strcmp(upper, "SET_EXPONENT") == 0)
            result_type = args[0].type == FVAL_ARRAY ? args[0].v.arr.elem_type : args[0].type;
    ...
    if (strcmp(upper, "SINH") == 0) {
        return make_real(sinh(val_to_real(args[0])));
    }
    if (strcmp(upper, "COSH") == 0) {
        return make_real(cosh(val_to_real(args[0])));
    }
    ...
    if (strcmp(upper, "LEN_TRIM") == 0) {
        if (args[0].type == FVAL_ARRAY && args[0].v.arr.elem_type == FVAL_CHARACTER) {
            ...
            for (int i = 0; i < args[0].v.arr.len; i++) {
                OfortValue elem = array_element_value(&args[0], i);
                long long len = 0;
                if (elem.type == FVAL_CHARACTER && elem.v.s) {
                    len = (long long)strlen(elem.v.s);
                    while (len > 0 && elem.v.s[len - 1] == ' ') len--;
                }
                free_value(&result.v.arr.data[i]);
                result.v.arr.data[i] = make_integer(len);
                free_value(&elem);
    ...
    if (strcmp(upper, "FINDLOC") == 0) {
        ...
        if (array_idx < 0) array_idx = 0;
        if (value_idx < 0) value_idx = 1;
        if (dim_idx < 0 && nargs >= 3 && (!arg_names || arg_names[2][0] == '\0')) dim_idx = 2;
        ...
        if (dim_idx >= 0) {
            ...
            int total = 1;
            OfortValue result;
            if (dim < 1 || dim > nd) ofort_error(I, "FINDLOC DIM is out of range");
            for (int d = 0; d < nd; d++) {
                if (d == dim - 1) continue;
                rdims[nrdims++] = array->v.arr.dims[d];
                total *= array->v.arr.dims[d];
            }
            if (nrdims == 0) {
                rdims[0] = 1;
                nrdims = 1;
            ...
            result = make_array(FVAL_INTEGER, rdims, nrdims);
            for (int out = 0; out < total; out++) {
                ...
                for (int d = 0; d < nd; d++) {
                    if (d == dim - 1) continue;
                    subs[d] = rem % array->v.arr.dims[d];
                    rem /= array->v.arr.dims[d];
                    rd++;
                }
                (void)rd;
                for (int pos = 0; pos < array->v.arr.dims[dim - 1]; pos++) {
                    int idx = 0;
                    int stride = 1;
                    int include = 1;
                    subs[dim - 1] = pos;
                    for (int d = 0; d < nd; d++) {
                        idx += subs[d] * stride;
                        stride *= array->v.arr.dims[d];
                    }
                    if (mask) {
                        if (mask->type == FVAL_ARRAY) {
                            OfortValue mv = array_element_value(mask, idx);
                            include = val_to_logical(mv);
                            free_value(&mv);
                        } else {
                            include = val_to_logical(*mask);
                        }
                    ...
                    if (include) {
                        ...
                        int eq = values_equal_fortran(I, elem, *value);
                        free_value(&elem);
                        if (eq) {
                            found_pos = pos + 1;
                            break;
            ...
            for (int d = 0; d < nd; d++) {
                ...
                if (found_index >= 0) {
                    ...
                    sub = (found_index / stride) % array->v.arr.dims[d] + 1;
                }
    ...
    if (strcmp(upper, "MAXLOC") == 0 || strcmp(upper, "MINLOC") == 0) {
        ...
        if (dim_idx >= 0) {
            ...
            int total = 1;
            OfortValue result;
            if (dim < 1 || dim > nd) ofort_error(I, "%s DIM is out of range", upper);
            for (int d = 0; d < nd; d++) {
                if (d == dim - 1) continue;
                rdims[nrdims++] = array->v.arr.dims[d];
                total *= array->v.arr.dims[d];
